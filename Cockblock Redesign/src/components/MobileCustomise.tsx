...
function Group({ className }: { className?: string }) {
  return (
    <div className={className}>
      <div className="absolute bottom-[52.2%] flex flex-col font-['Archivo_Narrow:Bold',sans-serif] font-bold justify-end left-0 right-0 text-[38px] top-0 tracking-[-0.76px]">
        <p className="leading-none">Customise Your Image</p>
      </div>
      <div className="absolute bottom-0 font-['Archivo_Narrow:Medium',sans-serif] font-medium leading-[1.5] left-0 right-[3.09%] text-[14px] top-[60.38%] tracking-[-0.14px]">
...
interface MobileCustomiseProps {
  onNavigate: (page: 'use' | 'merch' | 'info' | 'catalogue') => void;
  onToggleMenu: () => void;
  menuExpanded?: boolean;
}

export default function MobileCustomise({ onNavigate, onToggleMenu, menuExpanded = true }: MobileCustomiseProps) {
  return (
    <div className="bg-[#291a01] relative size-full overflow-hidden" data-name="mobile-customise-1">
      {menuExpanded ? (
        <>
      ...
      <Group className="absolute h-[159px] leading-[0] left-[calc(50%+0.5px)] text-center text-white top-[471px] translate-x-[-50%] w-[291px]" />
      <div className="absolute h-[50px] left-[calc(50%+2.5px)] top-[736px] translate-x-[-50%] w-[343px] cursor-pointer z-10" onClick={() => onNavigate('catalogue')}>
